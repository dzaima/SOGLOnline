...
String decompb(BigInteger inpbi) {
  ...
  while(!decompressable.equals(BI(0))) {
    ...
    if (nextIncludeAlphabet || eq==0 || eq==7) {
      ...
      lastD = length;
      if(logDecompressInfo) print("custom dictionary "+(nextIncludeAlphabet? "alphabet " : (eq==7?"long ":""))+"string with characters [");
      ...
      while (base > 1) {
        ...
        if (logDecompressInfo) print(", \""+compressableChars.charAt(compressableChars.length()-base+cc+1)+"\"");
        base-= cc+1;
      }
      if (nextIncludeAlphabet) {
    ...
    if (eq==5 | eq==4) {
      ...
      if(logDecompressInfo) for (int j = 0; j < CU.size(); j++) {
        String s = CU.get(j);
